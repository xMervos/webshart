...
class TestDatasetStats:
    ...
    def test_remote_dataset_cached_stats(self, mock_discover):
        ...
        dataset = discover_dataset('RareConcepts/conceptual-captions-12m-webdataset-indexed')
    ...
    def test_stats_with_empty_dataset(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "NoShardsFound" in str(exc_info.typename) or "No shards found" in str(exc_info.value)
    ...
    def test_large_dataset_performance(self, tmp_path):
        ...
        for i in range(num_shards):
            ...
            metadata = {
                "path": f"shard-{i:04d}.tar",
                "filesize": 1000000,
                "files": {f"file_{j}.dat": {"offset": j * 100, "length": 90} for j in range(10)}
            }
            json_path = tmp_path / f"shard-{i:04d}.json"
            json_path.write_text(json.dumps(metadata))
